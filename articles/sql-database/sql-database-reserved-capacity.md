...
|Quantity    |The amount of capacity being purchased within the SQL Database reserved capacity reservation. The quantity is the number of vCores in the selected Performance tier that are being reserved and will get the billing discount. For example, if you are running or planning to run SQL Database instances with the total capacity of 16 vCores in the East US region, then you would specify Quantity as 16 to maximize the benefit for all instances. |
